...
cookie и обезличенные технические данные, вебвизор выключен). Счётчик не
загружается до явного выбора посетителя; решение хранится только в его
браузере и меняется на странице политики конфиденциальности.
...
Локализованные social-карточки всех кейсов хранятся как готовые PNG/JPEG
1200×630. Их детерминированный исходник — `tools/build_og_cards.py`; для намеренной
пересборки нужен Pillow. Обычный CI читает размеры изображений без внешней
зависимости, проверяет точный файл для каждой RU/EN-страницы и не допускает
возврат кейсов на общую `og-default.png`.
...
## Воронка и цели Метрики

После согласия посетителя `site-config.js` отправляет технические этапы воронки.
Значения полей форм, контакты и описание задачи в Метрику не передаются.

| Идентификатор цели | Что означает |
|---|---|
| `case_open` | переход в кейс; параметры: кейс и размещение ссылки |
| `contact_open` | переход на страницу контакта |
| `form_start` | первое изменение поля формы, один раз за показ формы |
| `form_submit` | сервер подтвердил сохранение обычной заявки |
| `contact_channel` | переход в Telegram или email |
| `partner_open` | переход к партнёрской анкете |
| `partner_apply` | сервер подтвердил сохранение партнёрской анкеты |

Эти идентификаторы нужно один раз создать в интерфейсе счётчика как цели типа
«JavaScript-событие». Для основной составной цели используйте шаги
`case_open` → `contact_open` → `form_start` → `form_submit`. Код сайта сам по
себе не создаёт цели в кабинете Метрики.
...
Полная процедура чистого восстановления, перечень внешних секретов и границы
ответственности GitHub, DNS и Yandex Cloud зафиксированы в
[RECOVERY.md](RECOVERY.md). После значимого релиза обновляйте контрольный тег,
а полный Git mirror храните вне GitHub и не на том же физическом диске.
...
`npm test` также запускает `tools/check_secrets.py`: локальный гейт сообщает
только правило и путь подозрительного файла, не раскрывая найденное значение.
