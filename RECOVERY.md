...
`npm test` включает локальный поиск типовых ключей, токенов и чувствительных
имён файлов. Проверка не печатает найденные значения. На GitHub дополнительно
должны быть включены Secret scanning и Push protection: эти настройки не
восстанавливаются из содержимого репозитория автоматически.
