 ...
 *
 * Почему Vercel: форма на VPS (form.rednd.ru) не годится — тот же IP,
 * что и VPN-сервер, поэтому запросы таймаутятся у клиентов под VPN.
 *
 * Включение (см. README, раздел «Форма → Telegram (Vercel)»):
 *   1. vercel.com → Add New… → Project → импортировать этот репозиторий,
 *      имя проекта: rednd-form → Deploy
 *   2. Project → Settings → Environment Variables → добавить (Sensitive):
 *        TG_BOT_TOKEN — токен бота (тот же, что в /etc/rednd-form.env)
 *        TG_CHAT_ID   — chat_id (тот же)
 *      и передеплоить (Deployments → ⋯ → Redeploy)
 *   3. site-config.js → formEndpoint уже указывает на
 *      https://rednd-form.vercel.app/api/form — если Vercel выдал другой
 *      домен, поправьте одну эту строку.
 *
